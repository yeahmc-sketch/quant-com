...
def run_daily(trade_date_str, force=False):
    ...
    if positions:
        try:
            conn = sqlite3.connect(str(DB_PATH))
            first_entry = min(p['entry_date'] for p in positions.values())
            # 查询交易日天数（与is_rebalance一致）
            td_count = pd.read_sql(f"""
                SELECT COUNT(DISTINCT trade_date) FROM daily_kline
                WHERE trade_date >= '{first_entry}'
                  AND trade_date <= '{trade_date_str}'
            """, conn)
            trading_days_passed = int(td_count.iloc[0, 0])
            # 用交易日天数判断是否到期（而非日历天数）
            if trading_days_passed >= HOLD_DAYS:
                has_expiring = True
            cur = conn.cursor()
